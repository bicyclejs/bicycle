...
import parseArgs from '../legacyArgParser';
...
test('parses undefined args', () => {
  expect(parseArgs('(foo: "bar", bing: undefined)')).toEqual({
    foo: 'bar',
    bing: null,
  });
...
test('errors on missing closing bracket', () => {
  expect(() => parseArgs('(foo: "bar", bing: 10')).toThrowError(
...
test('errors on invalid JSON for attribute value', () => {
  expect(() => parseArgs('(foo: foo, bing: 10)')).toThrowError(
