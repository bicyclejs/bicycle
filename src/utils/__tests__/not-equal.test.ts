...
test('returns false when there is no difference', () => {
  ...
  expect(notEqual(freeze({foo: {bar: 10}}), freeze({foo: {bar: 10}}))).toBe(
    false,
  );
  expect(notEqual(freeze({foo: [1, 2, 3]}), freeze({foo: [1, 2, 3]}))).toBe(
    false,
  );
...
test('returns true when there is a difference', () => {
  ...
  expect(notEqual(freeze({foo: [1, 2, 3]}), freeze({foo: [1, 2, 4]}))).toBe(
    true,
  );
